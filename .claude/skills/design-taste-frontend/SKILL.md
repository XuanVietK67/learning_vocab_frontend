...
description: Anti-slop design taste for NEW frontend pages and components in the Lexa app — better layout, hierarchy, spacing rhythm, section variety, type scale, and motion choreography. Use when building or restyling any page/component under app/ or components/. It governs composition only; color, icons, fonts, and the global shell are owned by AGENTS.md + frontend-conventions.md, which OVERRIDE this skill (see Project Overrides at top of body).
---

## ⛳ PROJECT OVERRIDES — Lexa (READ THIS BEFORE ANYTHING BELOW)

This skill runs inside **Lexa**, a kawaii "Rainbow Cream" app. The project contract is
[AGENTS.md](../../../AGENTS.md) + [frontend-conventions.md](../../../frontend-conventions.md).
**Those two files WIN on every conflict with the rules below.** This skill contributes *composition
taste only*. Use it for everything in the table at the end of this block — nothing else.

**Hard overrides (the skill's defaults DO NOT apply here):**

- **Color → kawaii tokens only.** Never hardcode hex/rgb and never use raw Tailwind palette
  utilities (`bg-zinc-*`, `text-slate-*`, `bg-neutral-*`, etc.). Use the token classes from the
  AGENTS.md table: `bg-bg`/`bg-bg-soft`, `bg-card` + `border-line`, `text-ink`/`text-ink-2`,
  `text-muted-foreground`/`text-muted-2`, primary CTA = `bg-accent text-accent-foreground` (coral),
  soft fills `bg-accent-soft`/`text-accent`, decorative `var(--rainbow-1..5)`, status
  `text-success`/`text-warn`/`bg-danger-soft text-danger`. No arbitrary Tailwind values
  (frontend-conventions §5).
- **Icons → `lucide-react` only.** Ignore the skill's Phosphor/HugeIcons/Tabler preference.
- **Fonts → the project's existing fonts.** Ignore the skill's Geist/Outfit picks and its
  Inter/serif bans; do not introduce a new typeface.
- **Global shell → never touch.** Do NOT re-add `.kawaii-bg`, the `<StickerLayer />`, or any solid
  page background. New page roots wrap content in `relative z-[2]` (they sit above the global layers).
- **Motion → `motion` (motion/react), already installed.** Matches frontend-conventions §8. Keep the
  CSS animation classes (`animate-fade-up`, `animate-pop`, `animate-float`, `animate-shake`) for
  simple entrances; use motion/react for orchestrated/component motion. ALWAYS honor reduced motion
  (`useReducedMotion()` + the global `prefers-reduced-motion` rule — never bypass it with inline
  timings). The skill's ban on `window.addEventListener('scroll')` still holds.
- **Shape/depth → kawaii.** Cards `rounded-2xl`; buttons/inputs `rounded-xl`/`rounded-2xl`; pills
  `rounded-full`. Soft coral-tinted shadows, never hard grey drop shadows.

**Dials — re-pinned for kawaii (override the skill's 8 / 6 / 4 defaults):**

| Dial | Skill default | **Lexa baseline** | Why |
|---|---|---|---|
| DESIGN_VARIANCE | 8 | **5** | Playful, gentle asymmetry — not cold-luxury / edgy. |
| MOTION_INTENSITY | 6 | **4** | Soft, friendly motion that stays reduced-motion-safe. |
| VISUAL_DENSITY | 4 | **4** | Airy cream whitespace; raise toward 5–6 only on dense app screens. |

Tune per page, but start from the Lexa baseline, not the skill's defaults.

**Scope:** applies to all new frontend pages/components. On dense product surfaces (admin tables,
the learn player, multi-step forms) **frontend-conventions wins** over this skill's section-variety /
asymmetry rules — favor clarity over flourish there.

**What this skill governs here (and ONLY this):** layout & composition, visual hierarchy, spacing
rhythm, section variety, typographic *scale* (not the typeface), and motion *choreography*.
It does **not** decide color, icon set, fonts, or the global shell.
